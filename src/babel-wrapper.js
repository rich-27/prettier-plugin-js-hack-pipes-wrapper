/**
 * Prettier's babel parser does not respect the topic token from formatted code's babel config file.
 * To inject the hack-style pipeline operator topic token when we are able to determine it,
 * we need access to the internal variables of 'prettier/src/language-js/parse/babel.js'
 *
 * {get/set}BabelParserTopicToken() modify the pipelineOperatorPlugins array, which is the topic
 * token used during parsing.
 *
 * Note: We only modify pipelineOperatorPlugins[0] because that's the hack-style
 * pipeline operator plugin. Index [1] is the fsharp-style plugin.
 */

export function getBabelParserTopicToken() {
  return pipelineOperatorPlugins[0][1].topicToken;
}

export function setBabelParserTopicToken(topicToken) {
...
import {
  ...
  SOURCE_TYPE_MODULE,
  SOURCE_TYPE_SCRIPT,
} from '../vendor/prettier/src/language-js/parse/utils/source-types.js';
import wrapBabelExpression from '../vendor/prettier/src/language-js/parse/utils/wrap-babel-expression.js';
...
// END 'prettier/src/language-js/parse/babel.js'
